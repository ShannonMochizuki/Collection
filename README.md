...
## v16 — Isolated Collection PWA
This build is explicitly isolated for GitHub Pages project deployment at:

`https://shannonmochizuki.github.io/Collection/`

Changes:
- Manifest `id`: `/Collection/`
- Manifest `start_url`: `/Collection/`
- Manifest `scope`: `/Collection/`
- Service worker scope restricted to the Collection directory.
- Unique IndexedDB name: `collectionModelKitPortfolioDB`
- Unique service-worker cache namespace.
- Relative asset paths so this app does not fall back to the account root site.
- This prevents the Collection PWA from colliding with other apps such as `/Guitar/`.
